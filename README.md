...
Run `pwsh ./eng/Setup-ApiDocs.ps1` to acquire the latest `_NuGets`, `_DocsMedia`, and mdoc packages into `.artifacts/api-docs/packages`. By default, it uses the `dotnet-libraries-transport` and `dotnet-public` dnceng sources in `NuGet.Config`; pass `-PackageSource <local-NuGet-folder>` to use a local source containing all required artifacts. Then run `pwsh ./eng/Generate-ApiDocs.ps1`, which assumes that prepared package directory and required .NET reference packs already exist. The generator keeps only OpenPublishing infrastructure (`docfx.json`, filters, and breadcrumbs), regenerates framework moniker indexes, imports compiler XML documentation shipped in the NuGet packages, and promotes ECMA XML and media entirely from package content.

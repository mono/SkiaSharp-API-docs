...
Run `pwsh ./tools/Generate-ApiDocs.ps1` to replace the generated API tree and media from a clean staging directory. The script restores the latest `main` `_NuGets` and `_DocsMedia` transport packages, keeps only OpenPublishing infrastructure (`docfx.json`, filters, and breadcrumbs), and regenerates the framework moniker indexes. The promoted ECMA XML is the unmodified mdoc output with package-supplied media; source comments supply prose in a later phase.
